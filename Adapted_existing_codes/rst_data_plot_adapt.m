function [est,HDI,K]=rst_data_plot_adapt(Data,varargin)
...
for u=1:grouping
    ...
    if strcmpi(datascatter,'on')
        ...
        for p=1:size(Y,2)
            scatter(X(outliers(:,p)==0,p),Y(outliers(:,p)==0,p),S(find(outliers(:,p)==0)),color_scheme(u,:),'MarkerFaceColor','k'); hold on
            scatter(X(outliers(:,p)==1,p),Y(outliers(:,p)==1,p),(S(find(outliers(:,p)==1))),color_scheme(u,:),'MarkerFaceColor','k');
        end
...
%         clear xpoints filled
    ...
    plot(X(3:5),repmat(est(u),[1,length(X(3:5))]),'LineWidth',6,'Color','g');
